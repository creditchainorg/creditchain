...
The CLI reference pages under `docs/vocs/docs/pages/cli/` are **auto-generated** from the `creditchaind` binary's `--help` output. **Do not edit these files manually** — any hand edits will be overwritten and CI will fail regardless.
...
This builds `creditchaind` in debug mode and runs `docs/cli/update.sh` to regenerate all CLI pages. Commit the resulting changes.
