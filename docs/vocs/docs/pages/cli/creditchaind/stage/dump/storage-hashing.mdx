# creditchaind stage dump storage-hashing

`StorageHashing` stage

```bash
$ creditchaind stage dump storage-hashing --help
```
```txt
Usage: creditchaind stage dump storage-hashing [OPTIONS] --output-datadir <OUTPUT_PATH> --from <FROM> --to <TO>
