...
```bash
$ creditchaind node --help
```
```txt
Usage: creditchaind node [OPTIONS]
...
Options:
      ...
      --chain <CHAIN_OR_PATH>
          ...
          Built-in chains:
              mainnet, sepolia, holesky, hoodi, dev, local-single, local-multinode, devnet, testnet, creditchain-mainnet
...
RPC:
      ...
      --authrpc.jwtsecret <PATH>
          ...
          If no path is provided, a secret will be generated and stored in the datadir under `<DIR>/<CHAIN_ID>/jwt.hex`. For mainnet this would be `~/.local/share/creditchain/mainnet/jwt.hex` by default.
      ...
      --auth-ipc.path <AUTH_IPC_PATH>
          ...
          [default: <CACHE_DIR>d_engine_api.ipc]
...
Builder:
      --builder.extradata <EXTRA_DATA>
          ...
          [default: creditchaind/<VERSION>/<OS>]
