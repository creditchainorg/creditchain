# creditchaind snapshot-manifest

Generate a snapshot manifest from local archive files

```bash
$ creditchaind snapshot-manifest --help
```
```txt
Usage: creditchaind snapshot-manifest [OPTIONS] --source-datadir <SOURCE_DATADIR> --output-dir <OUTPUT_DIR>
