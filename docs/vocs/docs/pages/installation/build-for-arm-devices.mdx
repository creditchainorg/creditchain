---
description: Building and troubleshooting CreditChain on ARM devices.
---

# Building for ARM devices

CreditChain can be built for and run on ARM devices, but there are a few things to take into consideration before.
...
From [the Linux Kernel documentation](https://www.kernel.org/doc/html/v5.3/arm64/memory.html#:~:text=AArch64%20Linux%20uses%20either%203,for%20both%20user%20and%20kernel.), you can see that the memory layout with 4KB pages and a level-3 translation table limits the user space to 512GB, which is too low for CreditChain to sync on Ethereum mainnet.
...
If both your CPU architecture and the memory layout are valid, the instructions for building CreditChain will not differ from [the standard process](https://docs.creditchain.org/installation/source/).
...
> This error is documented [here](https://github.com/openibank/creditchain/issues/2211).
...
-   Wait for the process to finish, plug your storage device into your board and start it. You can now download or install CreditChain and it should work properly.
