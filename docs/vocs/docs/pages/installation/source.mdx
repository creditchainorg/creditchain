---
description: How to build, update, and troubleshoot CreditChain from source.
---

# Build from Source

You can build CreditChain on Linux, macOS, Windows, and Windows WSL2.

:::note
CreditChain does **not** work on Windows WSL1.
:::
...
These are needed to build bindings for CreditChain's database.
...
With Rust and the dependencies installed, you're ready to build CreditChain. First, clone the repository:

```bash
git clone https://github.com/creditchainorg/creditchain
cd creditchain
```

Then, install `creditchaind` into your `PATH` directly via:

```bash
cargo install --locked --path bin/reth --bin creditchaind
```

The binary will now be accessible as `creditchaind` via the command line, and exist under your default `.cargo/bin` folder.

Alternatively, you can build yourself with:

```bash
cargo build --release --bin creditchaind
```

This will place the node binary under `./target/release/creditchaind`, and you can copy it to your directory of preference after that.

Compilation may take around 10 minutes. Installation was successful if `creditchaind --help` displays the [command-line documentation](/cli/creditchaind).

If you run into any issues, please check the [Troubleshooting](#troubleshooting) section or open an issue in [openibank/creditchain](https://github.com/creditchainorg/creditchain/issues).

## Update CreditChain

You can update CreditChain to a specific version by running the commands below.

The `creditchain` directory will be the location you cloned CreditChain to during the installation process.
...
You can customise the compiler settings used to compile CreditChain via
[Cargo profiles](https://doc.rust-lang.org/cargo/reference/profiles.html).

CreditChain includes several profiles which can be selected via the Cargo flag `--profile`.
...
CreditChain will be installed to `CARGO_HOME` or `$HOME/.cargo`. This directory needs to be on your `PATH` before you can run `$ creditchaind`.
...
Rust Version (MSRV) which is listed under the `rust-version` key in CreditChain's
[Cargo.toml](https://github.com/creditchainorg/creditchain/blob/main/Cargo.toml).
