---
description: How to configure CreditChain using creditchain.toml and its options.
---

# Configuring CreditChain

CreditChain places a configuration file named `creditchain.toml` in the data directory specified when starting the node. It is written in the [TOML] format.
...
The stages section is used to configure how individual CreditChain sync stages behave, which has a direct impact on resource utilization and sync speed.

The defaults shipped with CreditChain try to be relatively reasonable, but may not be optimal for your specific set of hardware.
...
# folder = "/path/to/creditchain/era"
```
...
# dir = "/path/to/creditchain/etl-tmp"
