---
description: Troubleshooting common CreditChain node and database issues.
---
...
then most likely you're experiencing issues with the [database freelist](https://github.com/openibank/creditchain/issues/5228).
...
3. Run compaction (this step will take 5-6 hours, depending on the I/O speed)
    ```bash
    ./db-tools/mdbx_copy -c $(creditchaind db path) reth_compact.dat
    ```
4. Stop CreditChain
5. Backup original database
    ```bash
    mv $(creditchaind db path)/mdbx.dat reth_old.dat
    ```
6. Move compacted database in place of the original database
    ```bash
    mv reth_compact.dat $(creditchaind db path)/mdbx.dat
    ```
7. Start CreditChain
...
1. Stop CreditChain
2. Drop the database using [`creditchaind db drop`](/cli/creditchaind/db/drop)
3. Start reth
...
2. Check database integrity:
    ...
    cd reth
    make db-tools
    ./db-tools/mdbx_chk $(creditchaind db path)/mdbx.dat | tee mdbx_chk.log
    ```
    If `mdbx_chk` has detected any errors, please [open an issue](https://github.com/openibank/creditchain/issues) and post the output from the `mdbx_chk.log` file.
