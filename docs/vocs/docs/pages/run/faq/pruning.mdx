---
description: Pruning and full node options in CreditChain.
---

# Pruning & Full Node

:::info
Pruning and full node are new features of CreditChain,
and we will be happy to hear about your experience using them either
on [GitHub](https://github.com/openibank/creditchain/issues) or in the [Telegram group](https://t.me/paradigm_reth).
:::

By default, CreditChain runs as an archive node. Such nodes have all historical blocks and the state at each of these blocks
available for querying and tracing.

CreditChain also supports pruning of historical data and running as a full node. This chapter will walk through
the steps for running CreditChain as a full node, what caveats to expect and how to configure your own pruned node.

## Basic concepts

-   Archive node – CreditChain node that has all historical data from genesis.
-   Pruned node – CreditChain node that has its historical data pruned partially or fully through
    a [custom configuration](/run/configuration#the-prune-section).
-   Full Node – CreditChain node that has the latest state and historical data for only the last 10064 blocks available
...
To run CreditChain as a pruned node configured through a [custom configuration](/run/configuration#the-prune-section),
modify the `creditchain.toml` file and run CreditChain in the same way as archive node by following the steps from
the previous chapter on [how to run on mainnet or official testnets](/run/ethereum).

### Full Node

To run CreditChain as a full node, follow the steps from the previous chapter on
...
To run CreditChain in minimal storage mode, follow the steps from the previous chapter on
...
You can track the growth of CreditChain archive node size with our
