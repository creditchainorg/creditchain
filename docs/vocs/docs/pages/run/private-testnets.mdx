---
description: Running CreditChain in a private testnet using Kurtosis.
---

# Run CreditChain in a private testnet using Kurtosis

For those who need a private testnet to validate functionality or scale with CreditChain.
...
participants:
    - el_type: reth
      el_image: ghcr.io/openibank/creditchain
      cl_type: lighthouse
...
> [!TIP]
> If you would like to use a modified creditchaind node, you can build an image locally with a custom tag. The tag can then be used in the `el_image` field in the `network_params.yaml` file.
