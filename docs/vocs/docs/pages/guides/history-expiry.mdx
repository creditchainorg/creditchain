...
The import from ERA1 files within the pre-merge block range is included in the [creditchaind node](/cli/creditchaind/node) synchronization pipeline. It is disabled by default. To enable it, pass the `--era.enable` flag when running the [`node`](/cli/creditchaind/node) command.
...
If you want to import block headers and transactions from ERA1 files without running the synchronization pipeline, you may use the [`import-era`](/cli/creditchaind/import-era) command.
...
### Manual export
You can manually export block data from your database to ERA1 files using the [`export-era`](/cli/creditchaind/export-era) command.
