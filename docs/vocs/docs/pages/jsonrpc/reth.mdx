---
description: Reth compatibility namespace for balance changes and chain notifications.
---

# `reth` Namespace

The `reth` API is a compatibility namespace inherited from the Reth foundation. It is not part of the standard Ethereum JSON-RPC specification, but remains available for existing tooling that uses balance-change and chain-notification helpers.
