---
description: Overview of CreditChain's JSON-RPC API and namespaces.
---

# JSON-RPC

You can interact with CreditChain over JSON-RPC. CreditChain supports all standard Ethereum JSON-RPC API methods.

JSON-RPC is provided on multiple transports. CreditChain supports HTTP, WebSocket and IPC (both UNIX sockets and Windows named pipes). Transports must be enabled through command-line flags.
...
| [`reth`](/jsonrpc/reth)     | Compatibility namespace inherited from the Reth foundation for balance changes and chain notifications. | No        |
...
CreditChain supports HTTP, WebSockets and IPC.
...
To start an HTTP server, pass `--http` to `creditchaind node`:

```bash
creditchaind node --http
```
...
```bash
creditchaind node --http --http.addr 127.0.0.1 --http.port 12345
```

To enable JSON-RPC namespaces on the HTTP server, pass each namespace separated by a comma to `--http.api`:

```bash
creditchaind node --http --http.api eth,net,trace
```
...
```bash
creditchaind node --http --http.api all
```
...
```bash
creditchaind node --http --http.api none
```
...
```bash
creditchaind node --http --http.corsdomain https://mycoolapp.rs
```

Alternatively, if you want to allow any domain, you can pass `*`:

```bash
creditchaind node --http --http.corsdomain "*"
```
...
CreditChain creates a UNIX socket on Linux and macOS at `/tmp/creditchaind.ipc`. On Windows, IPC is provided using named pipes at `\\.\pipe\creditchaind.ipc`.
...
```bash
creditchaind node --http --http.api "admin,debug,eth,net,trace,txpool,web3,rpc,reth,ots,flashbots,miner,mev,testing"
```
