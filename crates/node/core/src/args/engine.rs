...
impl EngineArgs {
    ...
    pub fn tree_config(&self) -> TreeConfig {
        ...
        if self.legacy_state_root_task_enabled {
            tracing::warn!(target: "creditchaind::cli", "--engine.legacy-state-root has no effect anymore, use --engine.state-root-fallback to force synchronous state root computation");
        }
