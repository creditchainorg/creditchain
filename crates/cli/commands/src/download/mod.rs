...
const CREDITCHAIN_SNAPSHOTS_SOURCE: &str = "https://snapshots.creditchain.org (default)";
...
impl DownloadDefaults {
    ...
    pub fn default_download_defaults() -> Self {
        Self {
            available_snapshots: vec![
                Cow::Borrowed(CREDITCHAIN_SNAPSHOTS_SOURCE),
    ...
    fn mainnet_only_discovery(&self) -> bool {
        self.snapshot_api_url.trim_end_matches('/') == CREDITCHAIN_SNAPSHOTS_API_URL
    }
    ...
    pub fn with_snapshot_api_url(mut self, url: impl Into<Cow<'static, str>>) -> Self {
        ...
        for source in &mut self.available_snapshots {
            if source.as_ref() == CREDITCHAIN_SNAPSHOTS_SOURCE {
...
fn clear_existing_datadir(target_dir: &Path) -> Result<()> {
    ...
    info!(target: "creditchaind::cli", dir = ?target_dir, "Clearing existing snapshot data");
