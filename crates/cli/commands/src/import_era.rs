...
impl<C: ChainSpecParser<ChainSpec: EthChainSpec + EthereumHardforks>> ImportEraCommand<C> {
        ...
        N: CliNodeTypes<ChainSpec = C::ChainSpec>,
    {
        ...
        if let Some(path) = self.import.path {
            ...
            info!(target: "creditchaind::cli", ?era_type, path = %path.display(), to_block = ?self.to_block, with_receipts = self.with_receipts, "Starting ERA import");
            ...
            info!(target: "creditchaind::cli", ?era_type, %url, to_block = ?self.to_block, with_receipts = self.with_receipts, "Starting ERA import");
