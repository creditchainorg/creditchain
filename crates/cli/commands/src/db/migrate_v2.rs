...
impl Command {
    ...
    fn migrate_to_rocksdb<N: ProviderNodeTypes, T: Table>(
        ...
        info!(target: "creditchaind::cli", table = T::NAME, "Migrating MDBX table → RocksDB");
        ...
        info!(target: "creditchaind::cli", table = T::NAME, count, "MDBX table migrated to RocksDB");
        Ok(())
    }
    ...
    fn clear_migrated_and_recomputable_tables<N: ProviderNodeTypes>(
        ...
        info!(target: "creditchaind::cli", "Clearing migrated and recomputable MDBX tables");
